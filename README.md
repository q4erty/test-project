...
- В каждом микросервисе (`mc1`, `mc2`, `mc3`) добавлены зависимости:
  - Spring Actuator
  - Micrometer
  - Logstash-Logback
- Настроены конфигурации метрик и логирования, формат логов переведен в JSON для удобного парсинга.
- Добавлен `Dockerfile` в корень каждого микросервиса для упаковки в контейнер.
- Создан основной `docker-compose.yml` для всей инфраструктуры, включающий сервисы:
  - Prometheus, Grafana
  - Zookeeper, Kafka
  - MariaDB
  - Kafka-exporter, MariaDB-exporter
  - Микросервисы: `mc1`, `mc2`, `mc3`
  - Loki и Alloy
- Все сервисы работают в одной сети Docker (`observability-net`).
- В проект добавлен файл `.gitmodules` для корректной работы сабмодулей.
- В папке `monitoring` хранятся настройки метрик и алертов для Prometheus, Loki и Alloy.
- Добавлен `.env` файл для конфигурации портов, адресов и паролей (не хранится в репозитории для безопасности).
